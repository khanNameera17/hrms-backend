...
const { getAdditionalFields } = require("../../helpers/userHelper");
...
exports.getAllHierarchyUsersByFirm = async (req, res) => {
 try {
   ...
   // 1. Get Firm by name
   const firm = await Firms.findOne({ name: new RegExp(`^${firmName}$`, 'i') });
   ...
   for (const flowTypeId of firm.flowTypes) {
     const actorFlow = await ActorTypeHierarchy.findById(flowTypeId); // ✅ Correct model now
